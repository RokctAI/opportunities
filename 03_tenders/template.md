# Tender Opportunity: [Opportunity Title]

## Quick Stats
- **Tender Number**: [e.g., CDC/45/26]
- **Institution**: [Organ Of State]
- **Tender Type**: [Type]
- **Province**: [Province]
- **Date Published**: [YYYY-MM-DD]
- **Closing Date**: [YYYY-MM-DD - HH:MM]
- **Place Required**: [Full Address]

## Detailed Description
### Category
[Category Name]

### Tender Description
[Full description of the requirement]

### Special Conditions
[List of special conditions, downloading instructions, etc.]

## Briefing Session
- **Is there a briefing session?**: [Yes/No]
- **Is it compulsory?**: [Yes/No]
- **Briefing Date and Time**: [Date - Time]
- **Briefing Venue**: [Venue Details]

## Enquiries
- **Contact Person**: [Name]
- **Email**: [Email]
- **Telephone**: [Number]

## Documents & Links
- **Direct Link**: [Direct URL to Tender Portal or First Document]
- **Tender Documents**:
    - [File Name] (Uploaded: [Date])

## Audit & Status
- **Status**: ACTIVE
- **Last Verified**: [YYYY-MM-DD]

## AI Checklist (Jules)
<!-- This section is populated by Jules during enrichment. Format: - [ ] Subject | Due Date Offset (Days) -->
- [ ] Analyze Tender Documents for specific requirements | 1
- [ ] Identify Mandatory Compliance items | 2
- [ ] Prepare Initial Response Proposal | 3
